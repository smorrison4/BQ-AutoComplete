...
var bqac2LstToUse=bqac2LstOffice;
var bqac2LstRole=[];
var bqac2LstBU=[];
...
(function () {
	...
	if (bqac2ByAttrib==='Office') { bqac2LstToUse=bqac2LstOffice;}
	else if (bqac2ByAttrib==='Role') { bqac2LstToUse=bqac2LstRole;}
...
function bqac2AutoCompleteCategoryList() {
	...
	if (bqac2ByAttrib==='Office') {
		bqac2ByAttrib='Role';
		bqac2LstToUse=bqac2LstRole;
	}
	else if (bqac2ByAttrib==='Role') {
		bqac2ByAttrib='BU';
		bqac2LstToUse=bqac2LstBU;
	}
	else {
		bqac2ByAttrib='Office';
		bqac2LstToUse=bqac2LstOffice;
	}
	bqac2AutoCompleteFullOrCategories();
}
function bqac2AutoCompleteFullOrCategories() {
	var elem=document.getElementById(bqac2CtrlName);
	if (bqac2LstToUse.length < 41) { elem.bqItem = { ctrl: 'bqac2', selectedItem: '*', longName: '', dataChoice: '' }; }
...
function setBq2ByIndex(index) {
	if (index < 0 || index >= Bq2Lst.length) { return; }
	var item = Bq2Lst[index];
	setBq2ByItem(item);
}
function setBq2ByKey(key) {
	var jMax = Bq2Lst.length;
	var item = null;
